...
sub write_cron {
    ...
    for my $obj (@$objects) {
        ...
        my $replaced;
        ($cron, $replaced) = _substitute_existing_cron($cron, $name, $cron_row); # update
...
sub remove_cron_row {
    ...
    my ($cron) = _substitute_existing_cron(backup_cron(), $name, ''); # remove
    _write_out($cron);
...
sub _substitute_existing_cron {
    my ($target, $name, $replacement) = @_;

    my $replaced = $target =~ s/# $name: this cron was generated by Croon {{{.+# }}} $name\n/$replacement/sm;
